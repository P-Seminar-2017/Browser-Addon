class SQLHandler {
  ...
  static generateSQLList(id_where, sql_data, active, onUpdate) {
    ...
    for (var i = 0; i < sql_data.length; i++) {
      var d = new Date(Number.parseInt(sql_data[i].date));
      ...
      //Edit button
      var v2 = "edit-btn." + sql_data[i].id;

      function edit(event) {
        var innerID = event.target.id.split(".")[1];
        var old_text = document.getElementById("text-view." + innerID).innerHTML.split(">")[2].split("<")[0];
        ...
        document.getElementById(event.target.id).addEventListener("click", function(event) {
          var innerID2 = event.target.id.split(".")[1];
          var v3 = "new-text." + innerID2;
